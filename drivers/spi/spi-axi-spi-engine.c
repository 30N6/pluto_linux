						  ...
						  unsigned int index)
{
	...
	/* Use the adi,offloads phandle array to find the offload at index. */

	ret = fwnode_property_get_reference_args(spi->dev.fwnode, "adi,offloads",
						 NULL, 0, index, &ref);
	if (ret)
		return ERR_PTR(ret);

	ret = fwnode_property_read_u32(ref.fwnode, "reg", &reg);
	...
	if (reg >= SPI_ENGINE_MAX_NUM_OFFLOADS)
		return ERR_PTR(-EINVAL);

	offload = &spi_engine->offloads[reg];
				      ...
				      struct spi_message *msg)
{
	...
	struct spi_engine_program *p __free(kfree) = kzalloc(sizeof(*p) +
			sizeof(*p->instructions) * p_dry.length,  GFP_KERNEL);
	if (!p)
		return -ENOMEM;
					    ...
					    struct fwnode_handle *offload)
{
	...
	ret = fwnode_property_read_u32(offload, "reg", &index);
	if (ret)
		return ret;
	...
	return devm_add_action_or_reset(dev, spi_engine_offload_release, pdev);
}
					...
					struct device *dev)
{
	...
	fwnode_for_each_available_child_node(offloads, child) {
		ret = devm_spi_engine_register_offload(dev, child);
		if (ret)
